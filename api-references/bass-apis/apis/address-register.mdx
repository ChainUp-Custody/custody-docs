...
description: "请将币种地址发送给平台，地址通常用于分配给用户"
---

<Tip>
每次向Keysecure推送地址的数量上限为100条。在每次推送之前，我们会检查Keysecure剩余可用地址数是否大于5000。如果有5000个地址尚未被使用，则不允许注册。对于memo类型的地址，只需注册一个地址，memo将由乙方分配。
</Tip>
