...
为了让公链接入者更便捷、快速的接入Keysecure，我们提供了BaaS服务，以满足公链对接需求，平台基于过往客户的对接经验，梳理了一套对接方案。具体如下：
...
1. 可通过币种名称查询可用地址数量 注：（每次向Keysecure推送地址条数上限为`100`条，每次推送时会检查Keysecure剩余的地址可用数是否大于`5000`，如果有5000个地址没有被使用，则不允许注册；接口地址：`/api/v1/address/available`）
