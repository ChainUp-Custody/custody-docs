...
1. 运行程序下载   
  ...
  文件地址: https://github.com/Key-secure/mpc-co-signer/releases,  选择co-signer-linux-(version)-static版本，该版本Linux可直接运行
...
<Steps>
    ...
    <Step title="下载程序">
        ...
        文件地址: https://github.com/Key-secure/mpc-co-signer/releases,  选择co-signer-linux-(version) 版本，请按照后续步骤使用ego进行编译打包
...
7. -custody-pub-import        工具模式：输入启动密码，导入Keysecure的公钥，生成至keystore.json文件中
...
<Steps>
    ...
    <Step title="在conf目录下添加配置文件，文件名: config.yaml">
        ...
        ## 客户系统
        custom_service:
            ## 【可选】withdraw签名前回调客户系统地址确认交易是否正常，详情见: https://docs-cn.keysecure.io/api-references/mpc-apis/co-signer/callback/withdraw，未配置时需要强制验证sign
            withdraw_callback_url: ""
            ## 【可选】web3交易签名前回调客户系统地址确认交易是否正常，详情见: https://docs-cn.keysecure.io/api-references/mpc-apis/co-signer/callback/web3，未配置时需要强制验证sign
            web3_callback_url: ""
        ```
    </Step>
     ...
     **方式一：Co-Signer生成Rsa公私钥**
        ...
        私钥为请求Keysecure服务时加密请求参数使用，私钥对应公钥需配置到Keysecure, Keysecure使用该公钥解密**Co-Signer** 请求数据
    ...
    <Step title="Co-Signer 配置Keysecure公钥">
        ```sh
        ./co-signer -custody-pub-import 'Keysecure RSA Public key'
        ```

        执行如上命令后，按提示输入**Co-Signer** 启动密码。执行成功后conf目录下生成\更新文件：keystore.json

        该公钥由Keysecure提供，**Co-Signer** 使用该公钥解密Keysecure服务的返回数据
...
<Steps>
    <Step title="校验和下一步">
        👉 Co-signer 完成配置并成功启动后，服务仍处于未初始化状态。请前往APP执行私钥刷新操作，完成后服务才可正常使用。
    ...
    <Step title="登录APP生成私钥分片">
        登录APP，进入安全页面后选择Co-Signer开始配置，为工作站API创建私钥分片。
...
Keysecure也提供了更为便捷的部署方式，使用脚本一键式生成配置信息、以及运行环境
...
执行`install.sh`脚本，按照提示步骤，导入运行Co-Signer的必要信息和配置
...
echo "Please input Keysecure RSA public key:"
...
Keysecure需要访问**Co-Signer** 创建同权私钥、生成地址，故需要将**Co-Signer** 的启动端口对Keysecure的服务器IP添加安全组或白名单，以下是Keysecure的IP：

1. 入网：**Co-Signer** 服务器开通Keysecure服务器(IP: **54.254.7.206**)访问**28888端口**权限，注意28888端口为**Co-Signer** 程序监听端口，**可自定义**。

2. 出网：**Co-Signer** 服务器需访问Keysecure服务器，需开通访问**54.251.87.91:443**权限。