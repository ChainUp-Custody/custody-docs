...
Options:
    -CryptoSafe-pub-import string
            import CryptoSafe public key
    -h this is help
    -keystore-gen
...
7. -CryptoSafe-pub-import        工具模式：输入启动密码，导入CryptoSafe的公钥，生成至keystore.json文件中
...
<Steps>
     ...
     **方式一：Co-Signer生成Rsa公私钥**
        ...
        私钥为请求CryptoSafe服务时加密请求参数使用，私钥对应公钥需配置到CryptoSafe, CryptoSafe使用该公钥解密**Co-Signer** 请求数据
    ...
    <Step title="Co-Signer 配置CryptoSafe公钥">
        ```sh
        ./co-signer -CryptoSafe-pub-import 'CryptoSafe RSA Public key'
        ```

        执行如上命令后，按提示输入**Co-Signer** 启动密码。执行成功后conf目录下生成\更新文件：keystore.json

        该公钥由CryptoSafe提供，**Co-Signer** 使用该公钥解密CryptoSafe服务的返回数据
...
## III. **Co-Signer** 快捷部署脚本

CryptoSafe也提供了更为便捷的部署方式，使用脚本一键式生成配置信息、以及运行环境
...
CryptSafe需要访问**Co-Signer** 创建同权私钥、生成地址，故需要将**Co-Signer** 的启动端口对CryptSafe的服务器IP添加安全组或白名单，以下是CryptoSafe的IP：

1. 入网：**Co-Signer** 服务器开通CryptoSafe服务器(IP: **54.254.7.206**)访问**28888端口**权限，注意28888端口为**Co-Signer** 程序监听端口，**可自定义**。

2. 出网：**Co-Signer** 服务器需访问CryptoSafe服务器，需开通访问**54.251.87.91:433**权限。