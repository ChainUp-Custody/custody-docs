...
请工作站创建者登录[CryptoSafe Web端](https://www.cryptsafe.sg/)，在API管理页面创建API；

<div className="w-full flex items-center justify-center">
    <img src="/images/apis/api1cn.png" width="600"/>
</div>

### 关于RSA公钥
为了保证数据传输安全，CryptoSafe采用 `RSA-2048` **PKCS#8** 密钥格式的非对称加密算法对 API 请求和响应的数据进行加解密；其中公钥用于解密请求数据需要上传到CryptoSafe，私钥用于加密数据由客户自行配置到对应服务器并妥善保管；

### RSA公私钥对的说明
第一对RSA公私钥：创建 API 后，CryptSafe会默认生成，其中 `RSA公钥` 会给到开发者，开发者用于`响应参数解密`

第二对RSA公私钥：开发者生成，客户业务系统请求 API 前对参数加密，并将 `RSA公钥` 配置到CryptSafe用于`请求参数解密`

第三对RSA公私钥：开发者生成，Co-Signer请求 API 前对参数加密，并将 `RSA公钥` 配置到CryptSafe用于`请求参数解密`

第四对RSA公私钥（**可选**）：开发者生成，主要用于Co-Signer`未配置回调通知时验证交易签名`，该份公私钥由客户生成并保存，CryptSafe不持有
...
其中交易状态回调地址、签名回调是由客户在CryptoSafe平台配置，且由CryptoSafe主动触发，交易审批回调地址由客户在Co-Signer程序内配置，且由Co-Signer主动触发

* **交易状态回调**：在交易订单处理完成时（如充值、提现订单），CryptoSafe会回调客户通知订单最新状态，该回调地址由客户在CryptoSafe平台内，创建API页面配置【交易状态通知地址】

* **签名回调**：在API发起创建地址或为Co-Signer生成私钥分片时，CryptoSafe会回调Co-Signer进行签名操作；该回调地址由客户在CryptoSafe平台内，创建API页面配置【Co-Signer接口地址】
...
`工作站创建者`可在MPC系统【安全】-【Co-Signer】页面中创建Co-Signer私钥分片；创建的Co-Signer私钥分片将通过客户在CryptoSafe平台配置的 Co-Signer RSA公钥加密同步到Co-Signer服务器。
...
为了提高资金安全以及资金利用率，部分主链需要配置归集及加油功能，CryptoSafe为客户提供了一套自动化归集解决方案；具体操作流程如下：

登录[CryptoSafe Web端](https://www.cryptsafe.sg/)，在Co-Signer页面中创建；
