...
Co-Signer内置生成RSA公私钥命令，仅限Co-Signer使用，请参考[Co-Signer配置](/quick-start/mpc-wallet/api-integration/co-signer#co-signer-2)
</Tip>
...
Co-Signer支持部署在普通服务器或 `支持SGX` 的服务器，为了安全起见，建议使用带有 SGX 的服务器；推荐微软云服务商；[查看Co-Signer服务器配置要求](/quick-start/mpc-wallet/api-integration/co-signer#co-signer)
...
* [查看Co-Signer配置教程](/quick-start/mpc-wallet/api-integration/co-signer#co-signer-2)


### 创建Co-Signer私钥片段
完成API及Co-Signer程序配置后，在APP为Co-Signer创建私钥片段。

`工作站创建者`可在MPC系统【安全】-【Co-Signer】页面中创建Co-Signer私钥片段；创建的Co-Signer私钥片段将通过客户在ChainUp平台配置的 Co-Signer RSA公钥加密同步到Co-Signer服务器。
...
> 仅支持选择在APP端显示的金库；为了保证资金效率建议`归集金库`与`加油站`设置为不同的钱包（由于账户类型的主链出金是单笔执行，若配置成相同钱包，在为地址补充矿工费时，出金交易会等待直到补充矿工费完成。）
<div className="w-full flex items-center justify-center">
...
* 钱包成员（`任意角色`）配置金库归集、加油应用范围，币种归集、加油规则等信息
<div className="w-full flex items-center justify-center">
