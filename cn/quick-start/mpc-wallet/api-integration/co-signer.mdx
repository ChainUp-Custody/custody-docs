...
1. 运行程序下载   
  ...
  文件地址: https://github.com/ChainUp-Custody/mpc-co-signer/releases,  选择linux版本
...
<Steps>
    ...
    <Step title="下载程序">
        文件地址: https://github.com/ChainUp-Custody/mpc-co-signer/releases,  选择SGX版本
    </Step>

    <Step title="在工作目录下添加配置文件enclave.json, 配置文件内容如下">
        <Warning>
        注意：此配置文件规定co-signer只能在目录 /data/co-signer/下运行, 如需自定义运行目录, 请将配置中的"/data/co-signer/" 修改为自定义目录
        </Warning>
        ```json
        {
            "exe": "mpc-co-signer",
...
- **Co-Signer 合并版本v1.0.x中的文件，统一生成keystore文件**

该命令主要用于**合并v1.0.x**中的加密文件，使用v1.1.x版本以上可忽略

执行命令后按提示输入密码, 密码与配置钱包种子使用的密码一致！执行成功后conf目录下生成文件：keystore.json

```sh
./co-signer -keystore-gen
```
...
ChainUp Custody需要访问Co-Signer创建**同权私钥**、**生成地址**，故 `对接方` 需要将Co-Signer的启动端口对ChainUp Custody的服务器ip添加安全组或白名单，以下是Custody的Ip：

- openapi.hicoin.vip（**54.254.7.206**）
- openapi.hicoin.vip（**54.251.87.91**）