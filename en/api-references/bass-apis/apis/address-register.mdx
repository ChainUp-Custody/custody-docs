...
description: "Push address to ChainUp Custody Platform, this address is used to assign to users"
---

<Tip>
A maximum of 100 addresses per push to Custody, with each push checking that the number of addresses remaining available to ChainUp Custody is greater than 5,000 and disallowing registration if 5,000 addresses are not being used. If the address is memo type, only one address needs to be registered, and the memo will be allocated by Party B
</Tip>
