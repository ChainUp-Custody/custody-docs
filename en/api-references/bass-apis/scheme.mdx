...
1. The number of available addresses can be queried by coin name. Note: (A maximum of `100addresses` per push to Custody, with each push checking that the number of addresses remaining available toKeysecure is greater than 5,000 and disallowing registration if 5,000 addresses are not being used. Interface address: ``/api/v1/address/available`)
