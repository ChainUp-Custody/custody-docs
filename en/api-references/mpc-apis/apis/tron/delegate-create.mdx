...
<ParamField query="resource_type" type="integer" >
    Resource type to purchase: 0 for Energy and Bandwidth, 1 for Energy. Defaults to 0 if not specified.(Currently, only energy agents are supported.)
</ParamField>
<ParamField query="buy_type" type="integer">
    Resource purchase method: 0 for System Estimate, 1 for Specified Quantity. Defaults to 0 if not specified. (Currently, only a specified quantity is supported.)
</ParamField>
