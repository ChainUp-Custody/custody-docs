...
<Warning>
    ...
    3.Purchase TRON Network Transaction Resources

    4.Obtain [transaction records](/api-references/mpc-apis/apis/tron/delegate-record-list) for purchasing Tron resources and use status to determine if the resource has been successfully represented

    5.Use [Transaction Notification](/api-references/mpc-apis/notify/tron) timely obtain proxy status
...
<ParamField query="energy_num" type="integer" >
   Energy Amount.(Select 1. The specified quantity needs to be filled in, When for Specified Quantity, the minimum supported is 10,000)
</ParamField>
