...
<ParamField query="resource_type" type="integer" >
    Resource type to purchase: 0 for Energy and Bandwidth, 1 for Energy. Defaults to 1 if not specified.
 <Note> 
If you choose to purchase bandwidth, please note the following:

1. The minimum purchase quantity is 300.

2. Each bandwidth purchase order will incur a fixed additional fee of 560, which is used to cover the bandwidth cost required for system-initiated proxy and recovery. For example, if you request to purchase 600 units of bandwidth, the total bandwidth for this purchase order will be 600 + 560 = 1160.
</Note>
</ParamField>
...
2. If buy_type≠1, the quantity will be automatically provided by the system.
</Note>
</ParamField>
