...
<ResponseField name="fee_decimals" type="integer">BTC series, used for calculating the  native token's transaction fee. The formula is: `(fee_rate/10^fee_decimals)*size`</ResponseField>
...
<ResponseField name="gas_decimals" type="integer">ETH series, used for calculating the  native token's transaction fee. The formula is: `(gas_price/10^gas_decimals)*gas_limit`</ResponseField>
