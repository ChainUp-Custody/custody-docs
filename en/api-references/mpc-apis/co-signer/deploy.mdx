...
1. Run the program to download
  ...
  File address: https://github.com/Key-secure/mpc-co-signer/releases, select co-signer-linux-(version)-static version, this version can run directly on Linux.
...
```sh
wget https://github.com/Key-secure/mpc-co-signer/releases/download/v1.3.1/co-signer-linux-v1.3.1-static
```
...
<Step title="Download the program">
    ...
    File address: https://github.com/Key-secure/mpc-co-signer/releases, select version co-signer-linux-(version), please follow the subsequent steps to compile and package using ego.

    ```sh
    wget https://github.com/Key-secure/mpc-co-signer/releases/download/v1.3.1/co-signer-linux-v1.3.1
    ```
...
7. -custody-pub-import:              Tool mode: Input the startup password to import system's public key, stored in the keystore.json file.
...
<Steps>
    ...
    custody_service:
        ...
        domain: "https://openapi.keysecure.io/"
        ## [Optional] Request and response language, supporting zh_CN and en_US
        language: "zh_CN"

    ## Client System
    custom_service:
        ## [Optional] Withdrawal callback client system address for signature confirmation before signing, details see: https://docs-cn.keysecure.io/api-references/mpc-apis/co-signer/callback/withdraw, mandatory sign verification when not configured
        withdraw_callback_url: ""
        ## [Optional] Web3 transaction callback client system address for signature confirmation before signing, details see: https://docs-cn.keysecure.io/api-references/mpc-apis/co-signer/callback/web3, mandatory sign verification when not configured
        web3_callback_url: ""
    ```
    </Step>
 ...
 **Method One: Co-Signer Generates Rsa Public and Private Keys**
    ...
    The private key is used to encrypt request parameters when requesting Keysecure service, and the corresponding public key needs to be configured to system. System uses this public key to decrypt Co-Signer request data.
 ...
 **Method Two: Client Generates Rsa Private Key and Imports into Co-Signer**
    ...
    The private key is used to encrypt request parameters when requesting Keysecure service, and the corresponding public key needs to be configured to Keysecure. Keysecure uses this public key to decrypt Co-Signer request data

    </Step>
    <Step title="Configure public key for Co-Signer">
    ...
    This public key is provided by system, and Co-Signer uses this public key to decrypt the returned data from Keysecure services.
...
Keysecure also provides a more convenient deployment method, using scripts to generate configuration information and operating environments with one click
...
- **Complete initialization after startup**

<Steps>
    <Step title="Verification and next steps">
        👉 After the Co designer completes configuration and successfully starts, the service is still in an uninitialized state. Please go to the APP to perform the private key refresh operation. The service can only be used normally after completion。
         <div className="w-full flex items-center justify-left">
            <CardGroup cols={5}>
            <img src="/images/custody-guide/co_signer_log.png" width="1000"/>
            </CardGroup>
        </div>
    </Step>
...
echo ""
APP_ID=""
echo "App id can find in https://www.keysecure.io/mpc/center/api."
...
## Client System
custom_service:
    ## [Optional] Withdrawal callback client system address for signature confirmation before signing, details see: https://docs-en.keysecure.io/api-references/mpc-apis/co-signer/callback/withdraw, mandatory sign verification when not configured
    withdraw_callback_url: "${WITHDRAW_CALLBACK_URL}"
    ## [Optional] Web3 transaction callback client system address for signature confirmation before signing, details see: https://docs-en.keysecure.io/api-references/mpc-apis/co-signer/callback/web3, mandatory sign verification when not configured
...
echo ""
echo "Keysecure RSA public key can find in https://www.keysecure.io/mpc/center/api"
echo "Please input ChinaUp RSA public key:"
read KEYSECURE_PUBLIC_KEY
RESULT=$(echo ${PASSWORD} | ./co-signer -custody-pub-import ${KEYSECURE_PUBLIC_KEY})
echo $RESULT
STATUS=$?
if [ "$STATUS" != 0 ];then
  exit $STATUS
...
echo ""
echo "Custom RSA public key, use for verify withdraw data. https://docs-en.keysecure.io/api-references/mpc-apis/co-signer/flow#automatic-signature-signature-sign-verification-method"
...
CO_SIGNER_BIN=""
echo "Please input co-signer origin bin file name which download in https://github.com/Key-secure/mpc-co-signer/releases/:"
read CO_SIGNER_BIN
...
Keysecure needs access to the Co-Signer to create coequal private keys and generate addresses. Therefore, it's necessary to add the IP of the Co-Signer server to the security group or whitelist of the Keysecure server. Below are the IP addresses for Custody:

1. Inbound: Allow access from the Keysecure server (IP: **54.254.7.206**) to the Co-Signer server on port **28888**. Note that port 28888 is the listening port for the Co-Signer program and can be customized.

2. Outbound: The Co-Signer server needs to access the Keysecure server, so grant permission to access **54.251.87.91:443**.