...
1. Run the program to download
  ...
  File address: [https://github.com/Keysecure -Custody/mpc-co-signer/releases](https://github.com/Keysecure -Custody/mpc-co-signer/releases), select co-signer-linux-(version)-static version, this version can run directly on Linux.
...
```sh
wget https://github.com/Keysecure -Custody/mpc-co-signer/releases/download/v1.3.1/co-signer-linux-v1.3.1-static
```
...
<Step title="Download the program">
    ...
    File address: [https://github.com/Keysecure -Custody/mpc-co-signer/releases](https://github.com/Keysecure -Custody/mpc-co-signer/releases), select version co-signer-linux-(version), please follow the subsequent steps to compile and package using ego.

    ```sh
    wget https://github.com/Keysecure -Custody/mpc-co-signer/releases/download/v1.3.1/co-signer-linux-v1.3.1
    ```
...
"
    ...
    ## Client System
    custom_service:
        ## [Optional] Withdrawal callback client system address for signature confirmation before signing, details see: https://custodydocs-zh.Keysecure .com/api-references/mpc-apis/co-signer/callback/withdraw, mandatory sign verification when not configured
        withdraw_callback_url: ""
        ## [Optional] Web3 transaction callback client system address for signature confirmation before signing, details see: https://custodydocs-zh.Keysecure .com/api-references/mpc-apis/co-signer/callback/web3, mandatory sign verification when not configured
        web3_callback_url: ""
    ```
    </Step>
...
echo ""
APP_ID=""
echo "App id can find in https://keysecure.iompc/center/api."
...
echo ""
echo "Keysecure  RSA public key can find in https://keysecure.iompc/center/api"
echo "Please input ChinaUp RSA public key:"
read Keysecure _PUBLIC_KEY
RESULT=$(echo ${PASSWORD} | ./co-signer -custody-pub-import ${Keysecure _PUBLIC_KEY})
echo $RESULT
STATUS=$?
if [ "$STATUS" != 0 ];then
  exit $STATUS
...
echo ""
echo "Custom RSA public key, use for verify withdraw data. https://custodydocs-en.Keysecure .com/api-references/mpc-apis/co-signer/flow#automatic-signature-signature-sign-verification-method"
...
CO_SIGNER_BIN=""
echo "Please input co-signer origin bin file name which download in https://github.com/Keysecure -Custody/mpc-co-signer/releases/:"
read CO_SIGNER_BIN
