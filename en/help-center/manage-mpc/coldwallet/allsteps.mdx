...
Log in to the [Smart Contract Platform], select the asset network you want to manage, and connect your third-party wallet to initiate the contract creation transaction.
<Note>
Refer to [Create Contract](/user-guide/smart-contract-platform/create-Contract) for the contract creation process.


</Note>
If you're a co-manager of the cold wallet, add your third-party wallet address as a contract signatory.
You'll need to pay a miner fee through the connected wallet to create the contract.
Different network contracts might require different wallets. For instance, a TRON wallet contract can only be created using TronLink.
The created network contract can receive all token assets within that network.
</Step>
...
<Note>
If the workspace hasn’t selected a collection wallet, please configure it first in Auto Sweep. For more information, see [Asset Management](/user-guide/mpc-wallet/cold-wallet-manage/asset-manage).
</Note>
</Step>
...
For warm wallet contracts: Custody recommends adding cold contract and hot wallet addresses to the whitelist to ensure smooth storage and transfer of funds between hot and cold wallets.
For cold wallet contracts: Custody recommends adding hot contract and hot wallet addresses to the whitelist to ensure smooth storage and transfer of funds between warm and hot wallets.

1. og in to the Smart Contract Platform, select the network and contract to manage.
