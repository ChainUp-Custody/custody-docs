...
Upon configuring auto-sweep rules, the Keysecure System executes corresponding sweeping operations based on the specified sweeping strategy. This results in the assets on wallets/addresses within the sweeping scope being uniformly transferred to the designated sweeping wallet address. In cases where the wallet (withdrawal address) being swept lacks gas fees, the Gas Station will recharge the wallet's network native tokens according to the set refueling strategy.
...
An Omnibus Wallet is responsible for receiving assets scattered across different wallets into its address.
...
Scan the app QR code to log in to the Keysecure Web interface, use the 'Auto Sweep' function, select the omnibus wallet and gas station, and submit the configuration request.
</Step>
<Step title="Determining the Configuration">
The configuration of the omnibus wallet and gas station requires approval through [Workspace Strategy](/help-center/manage-mpc/strategy/workspcae).
</Step>
...
In this step, co-managers can select wallets under the workspace and initiate corresponding configurations. It is important to note that the Omnibus Wallet and Gas Station can‘t be the same.
...
o ensure the security of asset sweeping, both the omnibus wallet and the gas station need to be encrypted with private keys. This may result in longer approval and signing processes.
...
All sweeping and refueling orders will adhere to default refueling strategy: All sweeping and refueling orders will follow Keysecure System's default acceleration strategy to expedite sweeping completion.
