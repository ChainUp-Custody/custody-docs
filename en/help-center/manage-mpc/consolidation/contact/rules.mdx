...
##  Overview
Creating smart contracts and configuring auto sweeping rules through the Keysecure Web interface ensures that the system executes corresponding auto sweep operations according to the rules during the sweeping process. Simultaneously, by authorizing sub-wallets for sweeping contract in the App, you can precisely determine which sub-wallets have the right to sweep assets.

<Warning>
**Key Steps and Considerations**

Keysecure platform only supports `Web3 sub-wallets` for sweeping.
...
<Step title="Sign and Publish the Contract">
    The private key holder signs using the Keysecure App to scan the code, and the auto sweep rule is successfully published.
</Step>
</Steps>
...
<Step title="Open Sweep Tokens Feature">
    Log in and open the Keysecure App, switch to the MPC system, and open the "Discover - Sweep Tokens" function.
</Step>
...
Please use the Keysecure App's [Discover] - [Sweep Tokens] function, click to select the main wallet that has configured the sweeping contract for authorization.
</Warning>
