...
If an asset has never edited a transfer strategy, the transaction of the asset defaults to only require the workspace creator's final signature confirmation.
...
Currently, only the MPC workspace creator has the authority to manage the strategy engine. All editing and viewing functions of the strategy engine are only provided on the Custody App.
