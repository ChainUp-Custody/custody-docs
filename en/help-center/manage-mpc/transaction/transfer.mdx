...
<div className="w-full flex items-center justify-center">
    <img src="/images/help-center/withdrawal_cn.png" width="800"/>
</div>

**1. KYT Verification Pending Status**

After initiating a withdrawal, if you have opened and configured withdrawal addresses and KYT (Know Your Transaction) for the currency, the withdrawal enters the "KYT Verification Pending" status. The MPC wallet generates a KYT verification order to verify the KYT risk/service timeout of the withdrawal transaction. After manual confirmation, it enters the withdrawal approval stage.
...
<div className="w-full flex items-center justify-center">
    <img src="/images/help-center/deposit_cn.png" width="800"/>
</div>

**1. Unconfirmed Status**

When another address transfers digital currency to your MPC wallet address, the initial status is "Unconfirmed." After the transaction reaches the confirmation count, the status changes to "Confirmed."

**2. KYT Verification Pending Status**

If the deposit address and currency are configured with KYT (Know Your Transaction), the deposit transaction enters the "KYT Verification Pending" status. The wallet generates a KYT verification order and submits it to the KYT service provider for verification. Manual processing may be required at this stage, with the status being "KYT Manual Processing."

**3. KYT Manual Processing Status**

If the risk level returned by the KYT service provider requires manual processing, the deposit status is "KYT Manual Processing." After manual processing is completed, it moves on to the next stage.

**4. Confirmed Status**

If KYT is not configured, KYT verification passes, or KYT manual processing is completed, and the funds have been credited, the deposit status is "Completed." This concludes the deposit transaction.

**5. Failed/Exception Status**

If the transaction order is exceptional, such as a mainnet rollback, the transaction ends in a "Failed/Exception" status.
