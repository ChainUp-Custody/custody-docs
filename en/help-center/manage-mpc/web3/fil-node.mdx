...
To make it easier for FIL node owners to manage nodes without relying on Lotus commands, we have introduced the FIL node management feature in the MPC wallet. You only need to add the FIL address of the MPC wallet to the Owner or multisig Node's Owner, and you can easily manage the node through the interface. This provides a more convenient, low-threshold, and secure node management experience.



## II. FIL Node Management Workflow
...
The Filecoin mining pool management tool of Custody MPC wallet can be quickly accessed through the Filecoin mining pool management icon on the wallet interface.

If your wallet has opened the FIL currency, you can also click the Filecoin mining pool management tool button through the left swipe shortcut operation on the wallet list page to access the relevant functionality.
...
Currently, the MPC wallet supports three types of node operations: balance withdrawal, change owner, and change worker.
...
You can use the MPC wallet to directly withdraw the available balance of your FIL node, and the withdrawn funds will be transferred to the beneficiary or owner address of the node.
...
You can use the MPC wallet to change the Worker address of the node you manage.
...
The MPC wallet will present the relevant transaction operations of the nodes you manage as to-dos, waiting for you to process.
...
On the mainnet, multisig members in the MPC wallet can initiate proposals and sign proposal operations without distinguishing between initiating proposals and signing proposals. MPC signatures will mark the initiation of proposals and the signing of proposals. If all multisig members use the MPC wallet, there will be no situation where multiple proposals are initiated for the same transaction due to different tools.

When other multisig members use different tools to initiate multiple proposals for the same transaction, the MPC wallet will filter proposals to ensure that only one proposal is retained, avoiding duplicate proposals or duplicate signatures causing the transaction to succeed but the operation to fail.

</Step>
</Steps>


## III. Status Explanation

The Filecoin mining pool management tool of the MPC wallet includes various states:
