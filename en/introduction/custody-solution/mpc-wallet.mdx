---
title: Self Custody (MPC Wallet)
---

Keysecure, based on MPC (Secure Multi-Party Computation - TSS Threshold Signature Scheme) technology, has created a secure self-Keysecure wallet for institutional users. It provides multi-layer security, allowing for customized asset, private key management, and disaster recovery solutions.

- **All-in-One Asset Management Platform**

Keysecure supports users in managing MPC wallets on both desktop and mobile apps. The user-friendly platform allows secure storage and access to assets from over 200 main chains, providing powerful features with a simple and elegant interface accessible from anywhere.

<div className="w-full flex items-center justify-center">
    <img src="/images/introduction/mpc1en.PNG "width="600"/>
</div>

- **Distributed Key Storage Technology**

Keysecure employs distributed key storage technology, allowing users to generate and manage all private key shares. One key share is locally stored by the user, while the other two are stored on Amazon Cloud and Microsoft Cloud, respectively. This distributed storage, combined with hardware isolation technology, provides users with full control over their assets. 

Keysecure supports a 3-3 TSS configuration, where all three key shares participate simultaneously in the signature during a transaction, eliminating single points of failure in key management and significantly enhancing the security of self-managed assets, ensuring the highest level of security in the market.

- **Convenient Backup and Recovery**

The MPC System supports convenient private key management. Keysecure offers multiple backup options, including local recovery phrase backup and cloud backup. In case of a device change or local key loss, users can achieve one-step recovery. Keysecure ensures users have perpetual ownership of the created MPC System. If users no longer use Keysecure's services, they can use the offline recovery tool with the complete private key recovery phrase to import the wallet into another service.
...
<div className="w-full flex items-center justify-center">
    <img src="/images/introduction/mpctransfer-en.png" width="600"/>
</div>

- **Role-Based Operation Control**

Keysecure's MPC System provides role-based access control, allowing users to rigorously manage wallet permissions. Users can define roles such as approvers and regular members, effectively isolating information access and operational permissions for different sub-wallets based on their organizational structure. 
