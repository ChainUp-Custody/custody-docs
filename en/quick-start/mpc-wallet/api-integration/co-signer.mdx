...
<Steps>
    ...
    <Step title="Download program">
        File address: https://github.com/ChainUp-Custody/mpc-co-signer/releases,  Select co-signer-linux-(version) version, please follow the subsequent steps to use ego to compile and package.
...
<Steps>
    ...
    <Step title="Add a configuration file in the conf directory, file name: config.yaml">
        ...
        ## Customer system
        custom_service:
            ## withdraw Callback to the client system address before signing to confirm whether the transaction is normal, refer to: https://custodydocs-en.chainup.com/api-references/mpc-apis/co-signer/callback/withdraw , if not configured, it is necessary to force verification of sign.
            withdraw_callback_url: ""
            ## Web3 transaction signature callback client system address to confirm whether the transaction is normal, see: https://custodydocs-en.chainup.com/api-references/mpc-apis/co-signer/callback/web3 , if not configured, it is necessary to force verification of sign.
            web3_callback_url: ""
        ```
    </Step>
     ...
     **Method One: `co-signer` generates Rsa public and private keys** 
        ...
        After executing the command, enter the **Co-Signer** startup password as prompted. The password is the same as the one used for the wallet seed configuration! After successful execution, a file named keystore.json is generated in the conf directory.
     ...
     **Method Two: Customer generates and imports Rsa private key**

        After executing the command, enter the **Co-Signer** startup password as prompted. After successful execution, a file named keystore.json is generated in the conf directory.
  ...
  ```
     ...
     <Step title="Co-Signer configures Custody public key">
         After executing the command, enter the **Co-Signer** startup password as prompted.  After successful execution, a file is generated in the conf directory: keystore.json
