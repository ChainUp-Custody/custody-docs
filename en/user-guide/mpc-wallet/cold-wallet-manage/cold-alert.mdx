...
The CryptSafe system supports monitoring the following operational scenarios for cold wallet functions:

1. Balance Monitoring
Proposal transactions initiated by vote-free addresses do not require signer approval and are automatically passed. Since these transactions require the vote-free address to pay gas fees, it is necessary to maintain a certain amount of the native cryptocurrency. Insufficient native currency balance will cause transaction failure, affecting the automated scheduling of assets. The CryptSafe system monitors the balance of vote-free addresses on the warm wallet based on the user's configured contract network. If the balance falls below the threshold, an alert is triggered.
<Tip> 
...
The CryptSafe system monitors the status of warm-to-hot and hot-to-warm transfer tasks and notifies users of key events.
