...
CryptSafe's strategy engine allows workspace owner to enable and configure miner fee policies to prevent private key holders from maliciously or mistakenly setting excessively high custom miner fees, which could lead to financial losses.
...
* If a transaction requires a Co-Signer signature, the miner fee value will be automatically provided by the CryptSafe system with the best on-chain solution, avoiding the restrictions of the miner fee policy and preventing the transaction from being blocked.
...
<Step title="Login and Open Miner Fee Policy">
    Log in to the CryptoSafe App's MPC system, navigate to the "Security" - "Strategy Engine" function.
