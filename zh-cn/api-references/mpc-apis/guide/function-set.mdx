...
请钱包创建者登录[ChainUp控制台系统](https://custody.chainup.com/)，在API管理页面创建API；

<div className="w-full flex items-center justify-center">
    <img src="/zh-cn/images/apis/mpc-api1_cn.png" width="600"/>
</div>
...
Co-Signer内置生成RSA公私钥命令，仅限Co-Signer使用，请参考[Co-Signer配置](/zh-cn/quick-start/mpc-wallet/api-integration/co-signer)
</Tip>
...
* **签名回调**：在API发起创建地址或为Co-Signer生成私钥片段时，ChainUp会回调Co-Signer进行签名操作；该回调地址由客户在ChainUp平台内，创建API页面配置【Co-Signer接口地址】

* **交易审批回调地址**：请参考[Co-Signer配置](/zh-cn/api-references/mpc-apis/co-signer/callback/withdraw)
...
Co-Signer支持部署在普通服务器或 `支持SGX` 的服务器，为了安全起见，建议使用带有 SGX 的服务器；推荐微软云服务商；[查看Co-Signer服务器配置要求](/zh-cn/quick-start/mpc-wallet/api-integration/co-signer)
...
* [查看Co-Signer配置教程](/zh-cn/quick-start/mpc-wallet/api-integration/co-signer)


### II.III. 创建Co-Signer私钥片段
...
Co-Signer私钥片段丢失或泄露，可刷新出新的私钥片段或删除Co-Signer私钥片段；为了保证私钥片段安全，在刷新或删除后，Co-Signer原私钥片段不可用
</Warning>
...
<div className="w-full flex items-center justify-center">
    <img src="/zh-cn/images/apis/mpc-auto-collect1_cn.png" width="600"/>
</div>

* `钱包创建者`在APP审批签名；通过钱包私钥对设置的钱包进行加密，确保资金归集的地址正确。
<div className="w-full flex items-center justify-center">
    <img src="/zh-cn/images/apis/mpc-auto-collect2_cn.png" width="200"/>
</div>

* 钱包成员（`任意角色`）配置子钱包归集、加油应用范围，币种归集、加油规则等信息
<div className="w-full flex items-center justify-center">
    <img src="/zh-cn/images/apis/mpc-auto-collect3_cn.png" width="600"/>
</div>
